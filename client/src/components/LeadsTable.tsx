...
import { MoreHorizontal, Phone, MessageSquare, AlertCircle } from "lucide-react";
...
export function LeadsTable({ leads, isLoading }: LeadsTableProps) {
  ...
  const handleStatusChange = (id: number, currentStatus: string) => {
    // Simple cycle for demo purposes, could be a dropdown
            ...
            {leads.length === 0 ? (
              ...
              leads.map((lead) => (
                <tr key={lead.id} className="hover:bg-muted/30 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className={`flex items-center gap-1.5 text-xs ${urgencyStyles[lead.urgency as keyof typeof urgencyStyles] || urgencyStyles.Low}`}>
                  ...
                  <td className="px-6 py-4">
                    <button 
                      ...
                      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset transition-all hover:opacity-80 ${statusStyles[lead.status as keyof typeof statusStyles] || statusStyles.New}`}
                    >
                      {lead.status}
                    </button>
                  </td>
